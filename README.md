...
*Note, that clusters may have constraints around the number of replicas that are required for a topic. These will cause errors if not honoured. These errors may not clearly state the nature of the issue; instead referring to a general 'policy violation'.  Typically a production cluster will require at least three replicas. Container images running locally may  be configured to only accept a value of one.*
